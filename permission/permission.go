// /home/krylon/go/src/krylisp/permission/permission.go
// -*- mode: go; coding: utf-8; -*-
// Created on 11. 11. 2017 by Benjamin Walkenhorst
// (c) 2017 Benjamin Walkenhorst
// Time-stamp: <2017-11-11 04:59:44 krylon>

package permission

//go:generate stringer -type=FilePermission

// FilePermission is a "bitfield", i.e. a set of flags that indicate what permissions
// a given I/O handle has.
type FilePermission uint16

// I hope these are self-explanatory.
const (
	Read FilePermission = 1 << iota
	Write
	Append
	Sync
)
